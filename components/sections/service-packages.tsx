    ...
    <div className="space-y-8 md:space-y-10">
      ...
      {Object.entries(servicePackages).map(([serviceName, tiers]) => (
        <section key={serviceName} className="space-y-4">
          <h3 className="text-xl font-semibold md:text-2xl">{serviceName}</h3>
          <div className="grid gap-5 md:grid-cols-3">
            {tiers.map((tier) => (
              <article key={`${serviceName}-${tier.tier}`} className="rounded-xl border bg-white p-5 md:p-6">
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-accent">{tier.tier}</p>
                <h4 className="mt-2 text-lg font-semibold md:text-xl">Best for: {tier.bestFor}</h4>
